...
class Clear_WTF():
    def __init__(self):
        ...
        self.compare_file = './compare/wtf.csv'
        self.checklist_path = './wtf_checklist.txt'
    ...
    def deal_file(self):
        ...
        for root,dirs,files in os.walk(self.path):
            for i in range(len(files)):
                try:
                    if '_wtf' in files[i].split('.')[0] and files[i].split('.')[-1]=='txt':
    ...
    def total_file(self):
        ...
        datalist.to_csv(self.compare_path + 'wtf_result.csv', index=False)
        os.remove('wtf.csv')
    ...
    def compare_csv(self):
        try:
            if  os.path.exists(self.compare_file):
                ...
                new_data = pd.read_csv(self.compare_path+'wtf_result.csv',encoding='utf-8')
                ...
                df2.to_csv('./compare/end_wtf.csv',index=0)

                first = pd.read_csv('./compare/wtf.csv', encoding='utf-8')#读取第一个文件
                file1 = pd.DataFrame(first)
                # print(file1)
                end = pd.read_csv('./compare/end_wtf.csv', encoding='utf-8')#读取第二个文件
                file2 = pd.DataFrame(end)
                outfile = pd.merge(file1,file2,how='outer')
                outfile.to_csv('./compare/{}_wtf_result.csv'.format(self.dealtime), index=False,encoding='utf-8')#输出文件
                os.remove('./compare/end_wtf.csv')
        except:
            pass
...
if __name__ == "__main__":
    clear_wtf = Clear_WTF()
    clear_wtf.deal_file()
    clear_wtf.total_file()
    clear_wtf.compare_csv()
